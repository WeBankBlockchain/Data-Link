package com.webank.datalink.config;

import com.webank.datalink.amop.AmopMsgPrinter;
...
public class SdkBeanConfig {
        ...
        while(true){
            for(String certPath: possibilities ){
                try{
                    ...
                    configCryptoMaterial(property,"src/main/resources/conf");

                    ConfigOption configOption = new ConfigOption(property);
                    BcosSDK sdk = new BcosSDK(configOption);
                    Client client = sdk.getClient(config.getGroupId());
                    ...
                    sdk.getAmop().subscribeTopic("DB", new AmopMsgPrinter());
    ...
    public void configNetwork(ConfigProperty configProperty){
        ...
        List<String> peers = Arrays.asList(peerStr);
